...
>
    ...
    <nav class="mt-4 space-y-1 px-3 pb-10 text-sm">
        ...
        <x-nav-section-link :href="route('evaluation.report')" :active="request()->routeIs('evaluation.report') || request()->routeIs('evaluation.verify.*') || request()->routeIs('evaluation-criteria.*')">
        ...
        @if($user->isOfficer())
            @php($myGroup = $user->managedUnionGroups->count() === 1 ? $user->managedUnionGroups->first() : null)
            @if($myGroup)
                <x-nav-section-link :href="route('evaluation.self.edit', $myGroup)" :active="request()->routeIs('evaluation.self.*')">
                ...
                <x-nav-section-link :href="route('evaluation.members.edit', $myGroup)" :active="request()->routeIs('evaluation.members.*')">
